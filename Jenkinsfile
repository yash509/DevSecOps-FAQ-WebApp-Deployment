pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-FAQ-WebApp-Deployment.git'
            }
        }
        ...
        stage('OWASP File System SCAN') {
            steps {
                //dir('Band Website') {
                    dependencyCheck additionalArguments: '--scan ./ --disableYarnAudit --disableNodeAudit', odcfaq-appllation: 'DP-Check'
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag faq-app yash5090/faq-app:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/faq-app:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/faq-app:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/faq-app:latest'
                       sh 'docker-scout cves yash5090/faq-app:latest'
                       sh 'docker-scout recommendations yash5090/faq-app:latest'
                       sh 'docker-scout attestation yash5090/faq-app:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/faq-app:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Docker Scout Artifacts Analysis') {
            steps {
                script{
                  withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                      sh 'docker-scout sbom --format list yash5090/faq-app:latest' // docker-scout sbom yash5090/faq-app:latest
                    }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop faq-app | true"
                sh "docker rm faq-app | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name faq-app -p 5000:5000 yash5090/faq-app:latest' 
                //}
            }
        }
